...
pub enum ShapeError {
    ...
    MatmulIncompatible { left: Vec<usize>, right: Vec<usize> },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ShapeError::IncompatibleBroadcast { left, right } => {
                write!(
                    f,
                    "incompatible shapes for broadcast: {left:?} vs {right:?}"
                )
            }
            ...
            ShapeError::InvalidReshape { from, to } => {
                write!(
                    f,
                    "reshape: cannot reshape {from:?} to {to:?} (element count mismatch)"
                )
            }
        }
    }
